...
void sendMail(String ID, String Apellidos, String Nombre) {
  // Create a session
  ...
  try {
    ...
    // Fill the message
    messageBodyPart.setText("El empleado implicado es " + Apellidos + ", " + Nombre + " con ID: " + ID + ".");
...
// A function to send mail
void sendMailDesconocido(String ID) {
  // Create a session
  String host="smtp.gmail.com";
  Properties props=new Properties();
  ...
  try {
    ...
    BodyPart messageBodyPart = new MimeBodyPart();    
    // Fill the message
    messageBodyPart.setText("Se ha detectado una tarjeta extraña, con ID: " + ID + ".");
    Multipart multipart = new MimeMultipart();
    multipart.addBodyPart(messageBodyPart);
    msg.setContent(multipart);
    msg.setSentDate(new Date());
